...
description: Use when operating Releasepress release/export hygiene, report verification, local/private staging, or local promotion workflows; teaches the Releasepress product boundary and how to compose Remogram, Waylane, Skillpress, and other producer tools without replacing their authority.
---
...
Releasepress does not own product build logic, package registry credentials, release notes authority, package publishing authority, forge workflow authority, provider credentials, telemetry release readiness, workflow handoffs, Remogram forge facts, or Skillpress skill sync.
...
For normal Forest consumer use after public delivery, install the npm package
and smoke the repo-independent version command:
...
Use `npm install -g releasepress@<published-version>` or
`npx releasepress@<published-version> version --json` when automation must pin
the exact release. Do not point consumers at local Gitea remotes, workstation
paths, public-review refs, or stage repositories.

## Authority

- Releasepress owns release/export hygiene.
- `releasepress contract --json` owns Releasepress command capability discovery for governed invocation construction.
- Waylane can provide workflow execution facts when configured.
- Remogram owns forge facts and writes.
- Skillpress owns skill sync.

Do not treat skill prose or examples as machine command authority. Do not treat
a Releasepress receipt as merge, forge, package registry, or public release
authority.
...
releasepress evidence validate --json --manifest release.evidence_manifest.v1.json --release-target release-target.json
...
releasepress promote local --json --config releasepress.config.json --path /tmp/releasepress/local-evidence --source . --approve-local
```
...
lane state, forge credentials, network access, or telemetry sink access.
...
releasepress release deliver --json --config releasepress.config.json --path /tmp/releasepress/public-tree --target <provider-id> --approve-public <provider-id> --toolchain-compatibility toolchain.compatibility_matrix.v1.json
```
...
## Local Preparation and Promotion

For a source-backed local surface, configure `surfaces.local.prepare` with at
least one required explicit argv-array check and configure
`surfaces.local.smoke_check`.
Run `releasepress local prepare --json` against the exact clean source and a
dedicated evidence root, then pass that evidence root to `promote local`.

`local-prepare.json` binds the clean source SHA, normalized local policy,
command target, version, and check results. `promote local` re-resolves those
facts and runs the installed command smoke check from outside the source and
evidence trees before writing its receipt.

Do not use public export scan failures as local preparation evidence, and do
not treat local preparation as public readiness. When
`surfaces.local.source` is `export`, the existing public candidate checklist
and verify reports remain mandatory instead.

## Release Evidence Manifest

Use `releasepress evidence validate --manifest <file> --release-target <target.json> --json` to validate a neutral `release.evidence_manifest.v1` before provider delivery. Releasepress validates manifest shape, target binding, required public evidence kinds, privacy flags, and authority boundaries. The manifest producer is intentionally neutral; it does not need to be Atteway.

Provider delivery accepts `--evidence-manifest <file> --release-target <target.json>`. If `release_evidence.required: true` is configured, provider delivery blocks until a valid manifest and target are supplied either in config or on the command line.

Release evidence validation is not release readiness, artifact publication, producer truth, work judgment, forge truth, execution truth, replay truth, signing truth, security, or telemetry authority.

## Toolchain Compatibility

Use `releasepress toolchain validate --toolchain-compatibility <file> --json` to validate neutral `toolchain.compatibility_matrix.v1` policy evidence before provider delivery. If `toolchain_compatibility.required: true` is configured, provider delivery blocks until a valid matrix is supplied in config or with `--toolchain-compatibility`.

Releasepress validates this neutral Atteware-owned matrix without importing Atteway or producer runtime code. It does not inspect installed CLIs, run producer smokes, promote tools, or grant release readiness.
...
Use `git_host` providers for explicit git delivery targets such as GitHub, GitLab, local Gitea, or a custom remote. Require explicit `repo` and `ref`; do not infer `origin`, `main`, or provider defaults. Git-host launchers must use `launch_root: "export"` so they run from the scanned export tree. Direct git launcher argv must not force, mirror, or delete refs. Treat post-launch ref verification as receipt evidence, not publication authority.
...
When enabled, Releasepress emits one sanitized `diagnostic.event.v1` file for failed `export`, `scan`, `package`, `checklist`, `stage`, `verify`, `local prepare`, and `promote local` command packets, then sends it to the configured sink by replacing `{file}`. The sink owns storage, dedupe, fingerprinting, query, and reporting. Releasepress must not emit Waymark-owned event candidates or append sink-specific field flags.
...
.releasepress-report/local-prepare.json
.releasepress-report/local-promote.json
.releasepress-report/provider-<id>.json
.releasepress-report/release-status.json
```

`local-prepare.json` and `local-promote.json` are local evidence only.
`local-prepare.json` has type `releasepress_local_prepare`; the v1 promotion
receipt type is `releasepress_local_promote_receipt`. Both must claim only the
local surface and must not claim public GitHub, npm, or registry publication.
...
- Keep the `private-tooling` credential-file backstops active for `.env*`, `.npmrc`, `.netrc`, `id_rsa*`, `.aws/credentials`, and `*.pem` at root and nested paths.
