...
const TELEMETRY_COMMANDS = new Set(["export", "scan", "package", "checklist", "stage", "verify", "promote local"]);
...
export function buildTelemetryDiagnosticFields({ command, packet, sourceRoot = process.cwd() }) {
  ...
  const sourcePacketRef = safeText(packet.type ?? packet.error?.code ?? `releasepress_${command.replaceAll(" ", "_")}`);
  const fields = {
    ...
    scope: [
      ["command", commandRef],
      ["repo", safeRepoName(sourceRoot)],
      ["tool", "releasepress"]
    ],
    expected: `releasepress ${command} should complete without release-safety blockers.`,
    actual: summary.actual,
    evidenceRefs: evidenceRefs(command, commandRef),
    evidenceSnippets: summary.snippets,
    suggestedOwner: "releasepress",
  ...
  return {
    type: "releasepress_telemetry_field_candidate",
    command,
    fields,
    argv: fieldsToArgv(fields)
  };
}
...
function fieldsToArgv(fields) {
  ...
  for (const [key, value] of fields.scope) {
    argv.push("--scope", `${safeKey(key)}=${safeText(value)}`);
  }

  argv.push(
    "--expected",
    safeText(fields.expected),
    "--actual",
    safeText(fields.actual),
    "--summary",
    safeText(fields.summary, 240)
  );

  for (const [kind, value] of fields.evidenceRefs) {
    argv.push("--evidence-ref", `${safeKey(kind)}=${safeText(value)}`);
  }
  for (const [label, text] of fields.evidenceSnippets) {
    argv.push("--evidence-snippet", `${safeKey(label)}=${safeText(text, 1200)}`);
  }

  argv.push(
    "--suggested-owner",
    safeText(fields.suggestedOwner, 128),
    "--suggested-next-action",
    safeText(fields.suggestedNextAction),
    "--source-packet-ref",
    safeText(fields.sourcePacketRef, 256)
  );
...
function evidenceRefs(command, commandRef) {
  const refs = [["command", commandRef]];
  const report = REPORT_BY_COMMAND.get(command);
  if (report) {
    refs.push(["file", report]);
  }
  return refs;
}
...
function safeRepoName(sourceRoot) {
  const basename = path.basename(path.resolve(sourceRoot || "."));
  const safe = basename.replace(/[^A-Za-z0-9_.-]/g, "_").replace(/^\.+$/, "repo");
  return safe || "repo";
}
