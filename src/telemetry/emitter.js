...
export function emitTelemetryDiagnostic({
  ...
  try {
    const argv = [...telemetry.command_argv, ...candidate.argv];
    ...
    const eventId = parsed.value.event_id ?? parsed.value.event?.id ?? null;
    const fingerprint = parsed.value.fingerprint ?? parsed.value.event?.fingerprint ?? null;
