...
export function createExport({ sourceRoot = process.cwd(), outDir, config }) {
  ...
  if (sourceState.dirty === true) {
    throw new ReleasepressError(
      "export_dirty_source",
      "Public export requires a clean source checkout",
  ...
  for (const rel of selection.publicFiles) {
    copyStableFile({
      ...
      rel,
      codes: {
        unsafe: "not_regular_file",
        escape: "copy_path_escape",
        tooLarge: "export_file_too_large",
        changed: "export_source_changed",
        write: "export_copy_failed",
        noFollow: "export_copy_failed"
      }
    });
  }
  ...
  writeJson(path.join(reportDir, "candidate-fingerprint.json"), {
    ...releasepressEnvelope({ ok: true, type: "releasepress_candidate_fingerprint" }),
    ...candidate
  });
  ...
  return {
    ...releasepressEnvelope({ ok: true, type: "releasepress_export" }),
    source_root: root,
    out,
    source_commit: sourceState.commit,
...
export function stageExport({ exportRoot, sourceRoot = process.cwd(), config }) {
  if (!exportRoot) {
  ...
  const version = resolveVersion({ config, exportRoot: root, sourceRoot: resolvedSourceRoot });
  ...
  const output = {
    ...releasepressEnvelope({ ok: true, type: "releasepress_stage" }),
    export_root: root,
