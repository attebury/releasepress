...
const REQUIRED_SKILLPRESS_POLICY_PACKS = ["linter"];
const REQUIRED_CONTENT_POLICY_RULES = ["forbid_deprecated_diagram_telemetry_terms"];
const FORBIDDEN_SKILLPRESS_POLICY_PACKS = new Map([
  ["generic", "linter"],
  ["dogfood", "repo-owned content_policy.term_rules"]
]);
const REQUIRED_PHRASES = [
  ...
  "Waylane can provide workflow execution facts",
...
const REQUIRED_COMMANDS = [
  ...
  "releasepress toolchain validate --json",
...
function readSkillpressConfig(root) {
  ...
  const contentPolicy = normalizeContentPolicy(raw.content_policy);
  ...
  validateRequiredValues("content_policy.term_rules", contentPolicy.rule_ids, REQUIRED_CONTENT_POLICY_RULES);
...
function normalizeContentPolicy(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new ReleasepressError("skillpress_config_invalid", "skillpress.config.json content_policy must be configured", {
      field: "content_policy"
    });
  }
  if (!Array.isArray(value.term_rules) || value.term_rules.length === 0) {
    throw new ReleasepressError("skillpress_config_invalid", "skillpress.config.json content_policy.term_rules must be non-empty", {
      field: "content_policy.term_rules"
    });
  }
  const ruleIds = [];
  for (const [index, rule] of value.term_rules.entries()) {
    if (!rule || typeof rule !== "object" || Array.isArray(rule) || typeof rule.id !== "string" || rule.id.length === 0) {
      throw new ReleasepressError("skillpress_config_invalid", `content_policy.term_rules[${index}].id must be a non-empty string`, {
        field: `content_policy.term_rules[${index}].id`
      });
    }
