...
const TELEMETRY_OWNED_FIELD_FLAGS = new Set([
  "--producer",
  "--event-kind",
  "--classification",
  "--impact",
...
function normalizeTelemetryDiagnosticsConfig(raw) {
  ...
  if (commandArgv.includes("--file")) {
    throw new ReleasepressError(
      "invalid_config",
      "diagnostics.telemetry.command_argv must not include --file; Releasepress emits telemetry fields"
    );
  }
  if (commandArgv[1] === "event" && commandArgv[2] === "record") {
    const cliName = commandArgv[0] || "telemetry";
    throw new ReleasepressError(
      "invalid_config",
      `diagnostics.telemetry.command_argv must use ${cliName} event emit for field-based telemetry`
    );
  }
...
function assertTelemetryCommandPrefixArgv(commandArgv) {
  commandArgv.forEach((arg, index) => {
    ...
    if (TELEMETRY_OWNED_FIELD_FLAGS.has(flag)) {
      throw new ReleasepressError(
        "invalid_config",
        `diagnostics.telemetry.command_argv[${index}] must not set telemetry event fields; Releasepress appends bounded fields`
      );
    }
  });
}
...
function normalizePreflightConfig(raw) {
  ...
  const steps = normalizePreflightSteps(raw.steps ?? []);
...
function normalizePreflightSteps(rawSteps) {
  if (!Array.isArray(rawSteps)) {
    throw new ReleasepressError("invalid_config", "preflight.steps must be an array");
  }

  const seen = new Set();
  return rawSteps.map((step, index) => {
    if (!step || typeof step !== "object" || Array.isArray(step)) {
      throw new ReleasepressError("invalid_config", `preflight.steps[${index}] must be an object`);
    }

    const id = optionalEnum(step.id, `preflight.steps[${index}].id`, PREFLIGHT_STEP_IDS);
    if (seen.has(id)) {
      throw new ReleasepressError("invalid_config", "preflight step ids must be unique", { id });
    }
    seen.add(id);

    const argv = requireStringArray(step.argv, `preflight.steps[${index}].argv`, { min: 1 });
    const required = optionalBoolean(step.required ?? true, `preflight.steps[${index}].required`);
