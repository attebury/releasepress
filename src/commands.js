export const COMMANDS = [
  ...
  {
    name: "contract",
    summary: "Emit the Releasepress governed command capability manifest.",
    usage: "releasepress contract --json",
    requiredFlags: [],
    optionalFlags: [],
    approvalFlags: [],
    sideEffects: "none",
    reports: [],
    examples: ["releasepress contract --json"]
  },
  {
    name: "plan",
  ...
  },
  {
    ...
    summary: "Validate a neutral release evidence manifest against an explicit release target.",
    usage: "releasepress evidence validate --json (--manifest release.evidence_manifest.v1.json --release-target release-target.json|--evidence-bundle release-bundle)",
    requiredFlags: [],
    optionalFlags: ["--manifest", "--release-target", "--evidence-bundle"],
    approvalFlags: [],
    sideEffects: "read",
    reports: [],
    examples: ["releasepress evidence validate --json --manifest release.evidence_manifest.v1.json --release-target release-target.json", "releasepress evidence validate --json --evidence-bundle release-bundle"]
  },
  {
    name: "toolchain validate",
    summary: "Validate neutral toolchain compatibility policy evidence.",
    usage: "releasepress toolchain validate --json --toolchain-compatibility toolchain.compatibility_matrix.v1.json",
    requiredFlags: ["--toolchain-compatibility"],
    optionalFlags: [],
    approvalFlags: [],
    sideEffects: "read",
    reports: [],
    examples: ["releasepress toolchain validate --json --toolchain-compatibility toolchain.compatibility_matrix.v1.json"]
  },
  {
    name: "preflight",
    summary: "Run pre-release checks against an isolated exact candidate copy.",
  ...
  },
  {
    ...
    summary: "Add or update the local bin_shim surface policy on an existing config, without enabling public delivery.",
    usage: "releasepress bootstrap local --json --config releasepress.config.json --source . --bin-dir /path/to/bin",
    requiredFlags: ["--config", "--bin-dir"],
    optionalFlags: ["--source", "--command-name", "--force"],
    approvalFlags: [],
    sideEffects: "write",
    reports: [],
    examples: ["releasepress bootstrap local --json --config releasepress.config.json --source . --bin-dir /path/to/bin"]
  },
  {
    name: "local prepare",
    summary: "Prepare exact clean-source evidence for a source-backed local command.",
    usage: "releasepress local prepare --json --config releasepress.config.json --source <clean-source> --out <local-evidence-root>",
    requiredFlags: ["--config", "--source", "--out"],
    optionalFlags: [],
    approvalFlags: [],
    sideEffects: "write",
    reports: [
    ...
    examples: ["releasepress local prepare --json --config releasepress.config.json --source . --out ./local-evidence"]
  },
  {
    name: "promote local",
    summary: "Install a verified local command without claiming public delivery.",
    usage: "releasepress promote local --json --config releasepress.config.json --path <verified-evidence-root> --approve-local",
    requiredFlags: ["--config", "--path"],
    optionalFlags: ["--source"],
    approvalFlags: ["--approve-local"],
    sideEffects: "write",
    reports: [".releasepress-report/local-promote.json"],
    examples: ["releasepress promote local --json --config releasepress.config.json --path ./local-evidence --approve-local"]
  },
  {
    ...
    requiredFlags: ["--config", "--path"],
    optionalFlags: ["--source", "--approve-public", "--evidence-manifest", "--release-target", "--evidence-bundle", "--toolchain-compatibility"],
    approvalFlags: ["--approve-public"],
    sideEffects: "write",
