...
import { bootstrapLocalSurface } from "../src/bootstrap-local.js";
import { createChecklist } from "../src/checklist.js";
import { buildCapabilityManifest } from "../src/contract.js";
import { loadConfig, ReleasepressError, structuredError } from "../src/config.js";
import { validateReleaseEvidenceFiles } from "../src/evidence-manifest.js";
import { validateReleaseEvidenceBundleFiles } from "../src/evidence-bundle.js";
import { validateToolchainCompatibilityFiles } from "../src/toolchain-compatibility.js";
import { emitTelemetryDiagnostic } from "../src/telemetry/emitter.js";
import { buildTelemetryDiagnosticEvent, commandSupportsTelemetry } from "../src/telemetry/fields.js";
...
import { prepareLocal } from "../src/local-prepare.js";
...
function getNonFlags(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg.startsWith("--")) {
      ...
      if (key !== "json" && key !== "help" && key !== "approve-local" && key !== "approve-public-review" && key !== "force") {
        index += 1;
      }
      continue;
    }
...
function main(argv) {
  ...
  try {
    ...
    if (cmdName === "boundary") {
      result = getBoundary();
    } else if (cmdName === "contract") {
      result = buildCapabilityManifest();
    ...
    } else if (cmdName === "evidence validate") {
      result = flags.evidence_bundle
        ? validateReleaseEvidenceBundleFiles({ bundlePath: flags.evidence_bundle })
        : validateReleaseEvidenceFiles({
          manifestPath: flags.manifest,
          releaseTargetPath: flags.release_target
        });
      if (!result.ok) {
        process.exitCode = 1;
      }
    } else if (cmdName === "toolchain validate") {
      result = validateToolchainCompatibilityFiles({
        compatibilityPath: flags.toolchain_compatibility
      });
      if (!result.ok) {
        process.exitCode = 1;
      }
      ...
      result = deliverRelease({
        ...
        evidenceManifest: flags.evidence_manifest || null,
        releaseTarget: flags.release_target || null,
        evidenceBundle: flags.evidence_bundle || null,
        toolchainCompatibility: flags.toolchain_compatibility || null
      });
    ...
    } else if (cmdName === "bootstrap local") {
      result = bootstrapLocalSurface({
        sourceRoot: flags.source || process.cwd(),
        configPath: flags.config,
        binDir: flags.bin_dir,
        commandName: flags.command_name || null,
        force: flags.force === true
      });
    } else if (cmdName === "local prepare") {
      configForDiagnostics = loadConfig(flags.config);
      if (!flags.source) {
        throw new ReleasepressError("missing_source", "--source is required for local prepare");
      }
      sourceRootForDiagnostics = flags.source;
      result = prepareLocal({
...
function attachTelemetryDiagnostics({ command, result, config, sourceRoot }) {
  ...
  const candidate = buildTelemetryDiagnosticEvent({ command, packet: result, sourceRoot });
  if (!candidate) {
    return result;
  }
